...
- `Preferred refund address` is enforced server-side. When merchant wallets are preferred and a saved merchant refund wallet exists for the selected pay-in chain, the plugin overwrites payer-supplied `refundAddress` and `sourceAddress` values with that merchant wallet address. If the selected chain is enabled but no merchant refund wallet is saved for that chain, checkout falls back to asking the payer for a refund address and forwards that payer-provided address to MakePay. For best merchant-controlled refund routing, configure refund wallets for every chain you enable. When payer-entered refunds are selected, merchant wallets are used only for settlement/source-address routing and each BTCPay invoice checkout asks the customer for a refund address when needed.
